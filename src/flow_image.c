...
image make_integral_image(image im)
{
    ...
    float curr, new, left, top, corner;
    ...
    for (int x = 0; x < im.w; x++){
        for (int y = 0; y < im.h; y++){
            for (int c = 0; c < im.c; c++){
                ...
                // Get the left, top, and top-left corner pixels
                if (x == 0){
                    left = 0;
                }else{
                    left = get_pixel(integ, x-1, y, c);
                }
                if (y == 0){
                    top = 0;
                }else{
                    top = get_pixel(integ, x, y-1, c);
                }
                if (x == 0 && y == 0){
                    corner = 0;
                }else{
                    corner = get_pixel(integ, x-1, y-1, c);
                }

                // Set new pixel value
                new = curr + top + left - corner;
                set_pixel(integ, x, y, c, new);
            }
        }
    }
    return integ;
...
image box_filter_image(image im, int s)
{
    ...
    // Fill in box_filter_image so that every pixel in the output is the average of pixels in a given window size, s.
    for (i = 0; i < im.w; i++){
        for (j = 0; j < im.h; j++){
            for (k = 0; k < im.c; k++){

                // Get the top-left and bottom-right corners of the window
                x = i - s/2;
                y = j - s/2;
                x2 = i + s/2;
                y2 = j + s/2;

                // Make sure the window is within the image
                if (x < 0){
                    x = 0;
                }
                if (y < 0){
                ...
                float sum = get_pixel(integ, x2, y2, k) - get_pixel(integ, x, y2, k) - get_pixel(integ, x2, y, k) + get_pixel(integ, x, y, k);

                // Calculate the average of the window
                float avg = sum / (s*s);

                // Set the pixel in the output image
                set_pixel(S, i, j, k, avg);
            }
        }
    }
    free_image(integ);
...
image time_structure_matrix(image im, image prev, int s)
{
    ...
    // Step 1:
    // Calculate the spatial gradient using Sobel filters
    image Ix = make_gx_filter();
    image Iy = make_gy_filter();
    Ix = convolve_image(im, Ix, 0);
    Iy = convolve_image(im, Iy, 0);

    // Calculate the temporal gradient
    image It = sub_image(im, prev);

    // Step 2:
    // Calculate the strucutural components
    image IxIx = make_image(im.w, im.h, 1);
    image IyIy = make_image(im.w, im.h, 1);
    image IxIy = make_image(im.w, im.h, 1);
    image IxIt = make_image(im.w, im.h, 1);
    image IyIt = make_image(im.w, im.h, 1);
    for (i = 0; i < im.w * im.h; i++){
        IxIx.data[i] = Ix.data[i] * Ix.data[i];
        IyIy.data[i] = Iy.data[i] * Iy.data[i];
        IxIy.data[i] = Ix.data[i] * Iy.data[i];
        IxIt.data[i] = Ix.data[i] * It.data[i];
        IyIt.data[i] = Iy.data[i] * It.data[i];
    }

    // Step 3:
    // Combine the structure components into a single image
    image S = make_image(im.w, im.h, 5);
    for (int x = 0; x < S.w; x++){
        for (int y = 0; y < S.h; y++){

            // Set the structure components
            set_pixel(S, x, y, 0, get_pixel(IxIx, x, y, 0));
            set_pixel(S, x, y, 1, get_pixel(IyIy, x, y, 0));
...
image velocity_image(image S, int stride)
{
    ...
    for(j = (stride-1)/2; j < S.h; j += stride){
        for(i = (stride-1)/2; i < S.w; i += stride){
            ...
            // Multiply the matrix by the temporal vector to get the Velocity vector
            T.data[0][0] = -Ixt;
            T.data[1][0] = -Iyt;
