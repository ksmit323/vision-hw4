...
point project_point(matrix H, point p)
{
    ...
    // Project point as homography by filling in matrix
    c.data[0][0] = p.x;
    c.data[1][0] = p.y;
    c.data[2][0] = 1.0;
    ...
    // Turn homography back into points by dividing the third row
    point q = make_point(0, 0);
    q.x = out.data[0][0] / out.data[2][0];
    q.y = out.data[1][0] / out.data[2][0];
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
    ...
    for (i = 0; i < n; i++)
    {
    	...
    	// Calculate the distance between the projected point and the match
    	float dist = point_distance(p, m[i].q);

    	// If the distance is less than the threshold, then it is an inlier
    	if (dist < thresh)
        {
            // Swap the inlier with the first element in the array
            match temp = m[i];
            m[i] = m[count];
            m[count] = temp;
            count++;
	...
	for(k = 0; k < c.c; ++k){
        for(j = 0; j < c.h; ++j){
            for(i = 0; i < c.w; ++i){
                ...
                point p = project_point(H, make_point(i-abs(dx), j-abs(dy)));
